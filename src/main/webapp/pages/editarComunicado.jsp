        ...
        <form action="${pageContext.request.contextPath}/comunicados" method="post">
            ...
            <div class="mb-3">
                ...
                <input class="form-control" id="titulo" name="titulo" maxlength="200"
                       value="<%=comunicado.getTitulo() != null ? comunicado.getTitulo() : ""%>" required>
            </div>
            ...
            <div class="row g-3">
                <div class="col-md-4">
                    ...
                    <select class="form-select" id="categoria" name="idCategoria" required>
                        ...
                        %>
                        <option value="<%=cat.getIdCategoria()%>" <%=selecionado ? "selected" : ""%>><%=cat.getNome()%></option>
                        <%
                            }
                        }
                        %>
                ...
                <div class="col-md-4">
                    ...
                    <select class="form-select" id="prioridade" name="prioridade">
                        <option value="BAIXA" <%="BAIXA".equals(prioridadeAtual) ? "selected" : ""%>>BAIXA</option>
                        <option value="MEDIA" <%="MEDIA".equals(prioridadeAtual) ? "selected" : ""%>>MÉDIA</option>
                        <option value="ALTA" <%="ALTA".equals(prioridadeAtual) ? "selected" : ""%>>ALTA</option>
                    </select>
                </div>
                <div class="col-md-4">
                    <label class="form-label" for="status">Status</label>
                    <select class="form-select" id="status" name="status">
                        <option value="ATIVO" <%="ATIVO".equals(statusAtual) ? "selected" : ""%>>ATIVO</option>
                        <option value="INATIVO" <%="INATIVO".equals(statusAtual) ? "selected" : ""%>>INATIVO</option>
                    </select>
                </div>
            </div>
