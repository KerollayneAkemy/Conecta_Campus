...
<div class="page-header">
	<div>
		...
		<h1 class="page-title">
			<i class="bi bi-envelope-open-fill" aria-hidden="true"></i> Detalhe
			do Feedback
		</h1>
		<p class="page-subtitle">Visualize o conteúdo completo do feedback
			enviado.</p>
	</div>
	<a href="${pageContext.request.contextPath}/feedbackview?acao=listar"
...
<div class="row">
	...
	<div class="col-lg-4">
		<div class="card">
			<div class="card-body">
				<span class="metric-icon mb-3"><i class="bi bi-person-circle"
					aria-hidden="true"></i></span>
				<h3 class="h5">Remetente</h3>
				<% if (feedback.isAnonimo()) { %>
				<p class="page-subtitle mb-0">
					<i class="bi bi-incognito me-1" aria-hidden="true"></i> Envio
					anônimo
				</p>
				<% } else if (feedback.getUsuario() != null) { %>
				<p class="mb-0">
					<strong><%=feedback.getUsuario().getNome()%></strong>
				</p>
				<% } else { %>
		...
		<% if (podeExcluirFeedback) { %><a
			href="${pageContext.request.contextPath}/feedbackview?acao=excluir&id=<%=feedback.getIdFeedback()%>"
			class="btn btn-outline-danger w-100 mt-3" data-confirm="true"
			data-confirm-message="Deseja realmente excluir este feedback?"> <i
			class="bi bi-trash" aria-hidden="true"></i> Excluir feedback
		</a>
		<% } %>
	</div>
</div>
